...
describe('env decorator', () => {
  ...
  describe('array variable name', () => {
    it('should take first existing environment variable in a right order', () => {
      class Environment {
        @Env(['NON-EXISTING', 'TEST_TEST', 'BOOLEAN_FALSE_STRING'])
        test!: string;
      }
      const env = new Environment;
      expect(env.test).toBe('1');
